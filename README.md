...
- clone it and run npm install / yarn
- run `npm run build /yarn build` to test intial build
- add new component with `npm run generate "CompinentName"`/`yarn generate "CompinentName"`
- run `npm run story-dev/story-build `/`yarn story-dev/story-build ` to use [ladle, a storybook alternative that uses vite](https://ladle.dev/docs/)
- get npm access token (automation one)  github repository secret named `NPM_TOKEN` for publishing automation [publish script](.github\workflows\publish.yml)

> also remember to bump versions before publishing if it is not the first time  by running , `npm version major/minor/patch`
...
## detailed docs from original author
